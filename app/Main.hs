    ...
    Right size -> do
      case newBoard size of
        Right board -> do
          ...
          gameLoop GameState { currentBoard = board, currentSymbol = symbol, currentMove = 0 }
    ...
    case parsePosition input of
      Right position -> do
        case playMove board (Move symbol position) of
          Right board' -> gameLoop GameState { currentBoard = board', currentSymbol = otherSymbol symbol, currentMove = (move + 1) }
          Left playError -> do
